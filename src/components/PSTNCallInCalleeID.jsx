      ...
      {rules.length === 0 ? (
        ...
        <div style={{width:'100%'}}>
          <div style={{ ...tableContainerStyle, borderBottomLeftRadius:0, borderBottomRightRadius:0, borderBottom:'none' }}>
            <div style={{ ...blueBarStyle, borderBottom: '2px solid #888' }}>PSTN Call In CalleeID</div>
            <div ref={tableScrollRef} onScroll={handleTableScroll} style={{overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none'}}>
              <table style={{...tableStyle, minWidth: PSTN_CALL_IN_CALLEEID_TABLE_COLUMNS.length * 170 }}>
                <thead>
                  <tr>{PSTN_CALL_IN_CALLEEID_TABLE_COLUMNS.map(c => <th key={c.key} style={thStyle}>{c.label}</th>)}</tr>
                </thead>
                <tbody>
                  {pagedRules.map((item, idx) => {
                    ...
                    return (
                      <tr key={realIdx}>
                        ...
                        <td style={tdStyle}>{PSTN_CALL_IN_CALLEEID_FIELDS[1].options.find(opt => opt.value === item.callInitiator)?.label || item.callInitiator}</td>
      ...
      {isModalOpen && (
        <div style={modalOverlayStyle}>
          <div style={modalStyle}>
            <div style={modalHeaderStyle}>PSTN-&gt;IP CalleeID Manipulation</div>
