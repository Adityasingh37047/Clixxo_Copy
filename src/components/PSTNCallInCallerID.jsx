...
const PSTNCallInCallerID = () => {
  ...
  const handleOpenModal = (item = null, index = -1) => {
    setFormData(item ? { ...item, originalIndex: index } : PSTN_CALL_IN_CALLERID_INITIAL_FORM);
    setIsModalOpen(true);
  };
    ...
    <div className="bg-white min-h-screen flex flex-col items-center p-2 md:p-8">
      {rules.length === 0 ? (
        <div className="w-full h-full flex flex-col items-center justify-center" style={{ minHeight: '60vh' }}>
          <div className="text-gray-800 text-2xl md:text-[22px] font-semibold mb-8 text-center">No available number manipulation rule (PSTN Call In CallerID)!</div>
          <Button
            variant="contained"
            sx={{
              ...blueButtonSx,
            ...
            <div className="overflow-x-auto w-full border-b border-gray-300" style={{ borderBottomLeftRadius: 0, borderBottomRightRadius: 0, borderBottom: 'none' }}>
              ...
              >
                <table className="w-full min-w-[1400px] border border-gray-300 border-collapse whitespace-nowrap" style={{ tableLayout: 'auto', border: '1px solid #bbb' }}>
                  <thead>
                    <tr style={{ minHeight: 32 }}>{PSTN_CALL_IN_CALLERID_TABLE_COLUMNS.map(c => <th key={c.key} className="bg-white text-[#222] font-semibold text-[15px] border border-gray-300 text-center" style={{ border: '1px solid #bbb', padding: '6px 8px', minHeight: 32, whiteSpace: 'nowrap' }}>{c.label}</th>)}</tr>
                  </thead>
                  <tbody>
                    {pagedRules.map((item, idx) => {
                      ...
                      return (
                        <tr key={realIdx} style={{ minHeight: 32 }}>
                          ...
                          <td className="border border-gray-300 text-center bg-white" style={{ border: '1px solid #bbb', padding: '6px 8px', minHeight: 32, whiteSpace: 'nowrap' }}>{PSTN_CALL_IN_CALLERID_FIELDS[1].options.find(opt => opt.value === item.callInitiator)?.label || item.callInitiator}</td>
      ...
      <Dialog open={isModalOpen} onClose={handleCloseModal} maxWidth={false} PaperProps={{ sx: { maxWidth: '95vw', width: 440, background: '#f8fafd', borderRadius: 2, border: '2px solid #222' } }}>
        <DialogTitle className="bg-gradient-to-b from-gray-800 to-gray-600 text-white text-center font-semibold p-3 text-base">PSTN-&gt;IP CallerID Manipulation</DialogTitle>
        <DialogContent className="pt-3 pb-0 px-2" style={{padding: '12px 8px 0 8px'}}>
          <div className="flex flex-col gap-2 w-full">
            {PSTN_CALL_IN_CALLERID_FIELDS.map((field) => (
