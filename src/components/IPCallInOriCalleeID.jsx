      ...
      {rules.length === 0 ? (
        <div style={{ textAlign: 'center', marginBottom: '50vh' }}>
          <div style={{ color: '#222', fontSize: 22, marginBottom: 32 }}>
            No available number manipulation rule (IP Call In OriCalleeID)!
          </div>
          <button
            style={{
        ...
        <div style={{width:'100%'}}>
          <div style={{ ...tableContainerStyle, borderBottomLeftRadius:0, borderBottomRightRadius:0, borderBottom:'none' }}>
            <div style={{ ...blueBarStyle, borderBottom: '2px solid #888' }}>IP Call In OriCalleeID</div>
            <div ref={tableScrollRef} onScroll={handleTableScroll} style={{overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none'}}>
              <table style={{...tableStyle, minWidth: IP_CALL_IN_ORICALLEEID_TABLE_COLUMNS.length * 170 }}>
                <thead>
                  <tr>{IP_CALL_IN_ORICALLEEID_TABLE_COLUMNS.map(c => <th key={c.key} style={thStyle}>{c.label}</th>)}</tr>
                </thead>
                <tbody>
                  {pagedRules.map((item, idx) => {
                    ...
                    return (
                      <tr key={realIdx}>
                        ...
                        <td style={tdStyle}>{IP_CALL_IN_ORICALLEEID_FIELDS[1].options.find(opt => opt.value === item.callInitiator)?.label || item.callInitiator}</td>
      ...
      {isModalOpen && (
        <div style={modalOverlayStyle}>
          <div style={modalStyle}>
            <div style={modalHeaderStyle}>IP-&gt;PSTN OriCalleeID Manipulation</div>
