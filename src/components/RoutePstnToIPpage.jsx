...
const LOCAL_STORAGE_KEY = 'routePstnIpRules';
      ...
      {rules.length === 0 ? (
        ...
        <div style={{width:'100%'}}>
          <div style={{ ...tableContainerStyle, borderBottomLeftRadius:0, borderBottomRightRadius:0, borderBottom:'none' }}>
            <div style={{ ...blueBarStyle, borderBottom: '2px solid #888' }}>PSTN-&gt;IP Routing Rule</div>
            <div ref={tableScrollRef} onScroll={handleTableScroll} style={{overflowX: 'auto', scrollbarWidth: 'none', '-ms-overflow-style': 'none'}}>
              <table style={{...tableStyle, minWidth: (ROUTE_PSTN_IP_TABLE_COLUMNS.length + 2) * 150 }}>
                <thead>
                  <tr>
                    ...
                    {ROUTE_PSTN_IP_TABLE_COLUMNS.map(c => <th key={c.key} style={thStyle}>{c.label}</th>)}
...
return (
                      <tr key={realIdx}>
                        ...
                        {ROUTE_PSTN_IP_TABLE_COLUMNS.map(col => <td key={col.key} style={tdStyle}>{item[col.key] || '--'}</td>)}
      ...
      {isModalOpen && (
        <div style={modalOverlayStyle}>
          <div style={modalStyle}>
            <div style={modalHeaderStyle}>PSTN-&gt;IP Routing Rule</div>
